from domain.show import create_tv_show, get_an_aparitie, get_episoade
...
def add_show_to_manager(show_manager, the_show):
    """
    ...
    add_to_list(get_show_list(show_manager), the_show)
...
def test_add_show_to_manager():
    ...
    add_show_to_manager(test_manager, show)
    assert (len(get_show_list(test_manager)) == 1)

    show2 = create_tv_show('Suits', 2011, 134)
    add_show_to_manager(test_manager, show2)
    assert (len(get_show_list(test_manager)) == 2)


def test_delete_from_manager():
    test_manager = setup_show_manager(False)
    show1 = create_tv_show('See', 2019, 12)
    show2 = create_tv_show('Suits', 2011, 134)
    show3 = create_tv_show("The Walking Dead", 2010, 177)
    add_show_to_manager(test_manager, show1)
    add_show_to_manager(test_manager, show2)
    add_show_to_manager(test_manager, show3)
